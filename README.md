...
#### How requests are routed to the SitecoreDXG Generation Service: Ports, domains and IPs

Because SitecoreDXG runs off of an [express.js](https://expressjs.com/) server and is not registered with IIS, there is technically no way to actually bind an IP address or domain exclusively to the service on the server, alone, or to limit the service to a specific IP (without clunky logic that checks the requested domain on every request, that is - it was an implementation decision not to do that). By default, the service will listen for any requests that come into the machine on the specified port, regardless of the specific domain or IP address that was requested. See the optional instructions in Step 3 on setting up custom domains, dedicated IP addresses and removing the need to include a port number.
